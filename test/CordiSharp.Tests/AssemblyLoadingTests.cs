...
public class AssemblyLoadingTests
{
    ...
    public async Task ImportAccessor_ResolvesGeneratedContract()
    {
        ...
        // CordiSharpImportGenerator from the MULTIPLE [Import] attributes on ImportMarker —
        // no hand-written contracts; the generated bridges forward to the plugin services
        // loaded in the collectible ALC
    ...
    /// <summary>Annotations consumed by the import source generator (multiple imports on
    /// one type must each produce their own interface + accessor).</summary>
    [Import("greeter")]
    [Import("echo")]
